...
- **Errors treated alike.** Every external call (GitHub, the model, the queue) classifies its errors as retryable, rate-limited or permanent. A retry must be able to help, and a call that never answers must fail within a time limit: a webhook's enqueue gives up well before GitHub's 10 s delivery timeout. A failure a retry can fix is never cached as the answer or shown as an empty one. A rate-limited retry waits at least what the service's retry-after asks (#14, #22, #54, #116, #118, #119).
