...
describe("calculateBackoffDelay for a rate limit", () => {
  ...
  it("waits 5 minutes before each retry after a model rate limit", async () => {
    const rateLimited = await errorThrownFor("REVIEW_LLM_RATE_LIMITED");

    expect(calculateBackoffDelay(1, rateLimited)).toBe(5 * 60_000);
    expect(calculateBackoffDelay(2, rateLimited)).toBe(5 * 60_000);
  });
